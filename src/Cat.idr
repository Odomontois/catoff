...
record CatBase (ob: Type) where
    ...
    homEqv: (x, y: ob) -> Setoid (hom x y)

infixl 3 :==:
(:==:) :  {ob: Type} -> {base: CatBase ob} -> {a, b: ob} -> hom base a b -> hom base a b -> Type
(:==:)  {base} {a} {b} = seteq (homEqv base a b)

record CatLaws (ob: Type) (base: CatBase ob) where
    constructor MkCatLaws
    cat_l_unit : (a, b : ob) -> (f: hom base a b) -> compose base a b b (ident base b) f :==: f 
    cat_r_unit : (a, b : ob) -> (f : hom base a b) -> compose base a a b f (ident base a) :==: f
    cat_assoc : (a,b,c,d : ob) -> (f: hom base c d) -> (g: hom base b c) -> (h: hom base a b) -> 
        compose base a b d (compose base b c d f g) h :==: compose base a c d f (compose base a b c g h)

record Cat (ob: Type) where
    constructor MkCat
    catBase: CatBase ob
    catLaws: CatLaws ob catBase
