...
record Cat where
    ...
    l_unit : {a,b : ob} -> (f : hom a b) -> compose ident f = f 
    r_unit : {a, b : ob} -> (f : hom a b) -> compose f ident = f
    assoc : {a,b,c,d : ob} -> (f: 
...
Set : Cat 
Set = MkCat Type (\a, b => (a -> b)) id (\f,g => f . g) (\f => Refl) (\f => Refl) (\f, g, h => Refl)

record Functor (cod : Cat) (dom : Cat) where
    constructor MkFunctor
    mob : ob cod -> ob dom
    map : {a, b : ob cod} -> hom cod a b -> hom dom (mob a) (mob b)
    functor_id : {a : ob cod} -> map (ident cod {a = a}) = ident dom {a=mob a}
